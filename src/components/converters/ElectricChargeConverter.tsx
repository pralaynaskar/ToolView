...
const units = [
  ...
  { value: "mC", label: "mC (Millicoulomb)" },
  { value: "µC", label: "µC (Microcoulomb)" },
  { value: "nC", label: "nC (Nanocoulomb)" },
];
...
export default function ElectricChargeConverter() {
  ...
  const [toUnit, setToUnit] = useState("mC");
  const [, setHistory] = useLocalStorage<HistoryEntry[]>("calc-history", []);

  useEffect(() => {
    const num = parseFloat(inputValue);
    if (!isNaN(num)) {
      const result = convertUnit(num, 'electricCharge', fromUnit, toUnit);
