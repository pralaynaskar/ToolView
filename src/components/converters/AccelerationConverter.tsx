...
const units = [
  { value: "m/s2", label: "m/s² (Meter/second²)" },
  { value: "km/s2", label: "km/s² (Kilometer/second²)" },
  { value: "g", label: "g (Standard gravity)" },
];

export default function AccelerationConverter() {
  const [inputValue, setInputValue] = useState<string>("9.80665");
  ...
  useEffect(() => {
    ...
    if (!isNaN(num)) {
      const result = convertUnit(num, 'acceleration', fromUnit, toUnit);
