...
const units = [
  { value: "K/m", label: "K/m (Kelvin/meter)" },
  { value: "C/m", label: "°C/m (Celsius/meter)" },
  { value: "F/ft", label: "°F/ft (Fahrenheit/foot)" },
];

export default function TemperatureGradientConverter() {
  const [inputValue, setInputValue] = useState<string>("1");
  const [outputValue, setOutputValue] = useState<string>("");
  const [fromUnit, setFromUnit] = useState("K/m");
  const [toUnit, setToUnit] = useState("F/ft");
  const [, setHistory] = useLocalStorage<HistoryEntry[]>("calc-history", []);

  useEffect(() => {
    const num = parseFloat(inputValue);
    if (!isNaN(num)) {
      const result = convertUnit(num, 'temperatureGradient', fromUnit, toUnit);
