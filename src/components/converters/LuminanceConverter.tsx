...
const units = [
  { value: "cd/m2", label: "cd/m² (Candela/m²)" },
  { value: "nit", label: "nit (Nit)" },
  { value: "apostilb", label: "Apostilb" },
  { value: "lambert", label: "Lambert" },
  { value: "foot-lambert", label: "Foot-lambert" },
];

export default function LuminanceConverter() {
  const [inputValue, setInputValue] = useState<string>("100");
  const [outputValue, setOutputValue] = useState<string>("");
  const [fromUnit, setFromUnit] = useState("cd/m2");
  const [toUnit, setToUnit] = useState("foot-lambert");
  const [, setHistory] = useLocalStorage<HistoryEntry[]>("calc-history", []);

  useEffect(() => {
    const num = parseFloat(inputValue);
    if (!isNaN(num)) {
      const result = convertUnit(num, 'luminance', fromUnit, toUnit);
