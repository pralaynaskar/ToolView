...
export default function ImageToolsPage() {
  const allImageTools = useMemo(() => {
    return TOOLS.filter(tool => tool.category === 'Image Tools');
  }, []);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-6 p-4 bg-card border rounded-xl">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary">Image Tools</h1>
          <p className="text-muted-foreground mt-1">This section includes some helpful tools that can use it to change your image's form.</p>
        </div>
      </div>
      
      {allImageTools.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          ...
          <div className="text-center py-12 text-muted-foreground">
            <h3 className="text-lg font-semibold">No Image Tools Found</h3>
