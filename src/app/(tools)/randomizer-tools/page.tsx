...
export default function RandomizerToolsPage() {
  const allRandomizerTools = useMemo(() => {
    return TOOLS.filter(tool => tool.category === 'Randomizer Tools');
  }, []);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-6 p-4 bg-card border rounded-xl">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary">Randomizer Tools</h1>
          <p className="text-muted-foreground mt-1">This section provides a number of randomizer tools to assist you in making decisions.</p>
        </div>
      </div>
      
      {allRandomizerTools.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {allRandomizerTools.map((tool) => (
          ...
          <div className="text-center py-12 text-muted-foreground">
            <h3 className="text-lg font-semibold">No Randomizer Tools Found</h3>
