...
export default function DevelopmentToolsPage() {
  const allDevTools = useMemo(() => {
    return TOOLS.filter(tool => tool.category === 'Development Tools');
  }, []);

  const groupedTools = useMemo(() => {
    return allDevTools.reduce((acc, tool) => {
      ...
      return acc;
    }, {} as Record<string, typeof allDevTools>);
  }, [allDevTools]);
  
  const subCategoryOrder = ['Converters', 'Beautify', 'Analyse', 'GitHub'];
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-6 p-4 bg-card border rounded-xl">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary">Development Tools</h1>
          <p className="text-muted-foreground mt-1">This section enables you to create an organized page for your codes using development tools.</p>
        </div>
      </div>
      ...
      <div className="space-y-10">
        {displayedSubCategories.length > 0 ? (
          ...
          <div className="text-center py-12 text-muted-foreground">
            <h3 className="text-lg font-semibold">No Development Tools Found</h3>
