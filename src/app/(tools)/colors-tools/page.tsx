...
export default function ColorsToolsPage() {
  const allColorsTools = useMemo(() => {
    return TOOLS.filter(tool => tool.category === 'Colors Tools');
  }, []);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-6 p-4 bg-card border rounded-xl">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary">Colors Tools</h1>
          <p className="text-muted-foreground mt-1">This tool provides you with several choices of colors tools which you can use them to handle with colors.</p>
        </div>
      </div>
      
      {allColorsTools.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {allColorsTools.map((tool) => (
          ...
          <div className="text-center py-12 text-muted-foreground">
            <h3 className="text-lg font-semibold">No Colors Tools Found</h3>
