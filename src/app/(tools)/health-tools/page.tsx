...
export default function HealthToolsPage() {
  const allHealthTools = useMemo(() => {
    return TOOLS.filter(tool => tool.category === 'Health Tools');
  }, []);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-6 p-4 bg-card border rounded-xl">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary">Health Tools</h1>
          <p className="text-muted-foreground mt-1">This section contains a number of different tools to support your wellness journey.</p>
        </div>
      </div>
      
      {allHealthTools.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {allHealthTools.map((tool) => (
          ...
          <div className="text-center py-12 text-muted-foreground">
            <h3 className="text-lg font-semibold">No Health Tools Found</h3>
