...
export default function PdfToolsPage() {
  const allPdfTools = useMemo(() => {
    return TOOLS.filter(tool => tool.category === 'PDF Tools');
  }, []);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-6 p-4 bg-card border rounded-xl">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary">PDF Tools</h1>
          <p className="text-muted-foreground mt-1">This section includes some helpful tools to manage your PDF files.</p>
        </div>
      </div>
      
      {allPdfTools.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          ...
          <div className="text-center py-12 text-muted-foreground">
            <h3 className="text-lg font-semibold">No PDF Tools Found</h3>
